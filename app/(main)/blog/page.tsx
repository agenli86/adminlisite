...
async function getData() {
  ...
  const [blogsRes, heroRes] = await Promise.all([
    supabase.from('blogs').select('*').eq('is_active', true).order('created_at', { ascending: false }),
    supabase.from('page_settings').select('*').eq('page_key', 'blog').single()
  ])
  return { 
    blogs: blogsRes.data || [],
    hero: heroRes.data
    ...
    <>
      ...
      <section className="relative bg-gradient-to-br from-primary-600 to-primary-900 text-white py-20 overflow-hidden">
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          <h1 className="text-4xl lg:text-5xl font-bold mb-4">{hero?.hero_title || 'Blog'}</h1>
        ...
        <div className="container mx-auto px-4">
          ...
          {categories.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-8 justify-center">
              <span className="bg-primary-600 text-white px-4 py-2 rounded-full text-sm font-medium">Tümü</span>
              {categories.map((cat) => (
                <span key={cat} className="bg-white text-gray-700 px-4 py-2 rounded-full text-sm font-medium hover:bg-primary-100 cursor-pointer">
                  {cat}
                </span>
              ))}
            </div>
          ...
          {/* Blog Grid */}
          {blogs.length === 0 ? (
            <div className="text-center py-12 text-gray-500">Henüz blog yazısı yok.</div>
          ) : (
                ...
                >
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    <div className="flex items-center gap-3 text-sm text-gray-500 mb-3">
                      {blog.category && (
                        <span className="bg-primary-100 text-primary-700 px-2 py-1 rounded">{blog.category}</span>
                      )}
                    ...
                    <p className="text-gray-600 line-clamp-3 mb-6 flex-grow">{blog.excerpt}</p>
                    <Link 
                      href={`/blog/${blog.slug}`}
